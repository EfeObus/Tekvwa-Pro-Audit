...
`wht_credit_notes` — done, verified (check `docs/REMEDIATION_LOG.md` for current deploy status of
the most recent one). 57 of the 66 tables in the original scope remain (see the tier tables above
for the full list; done tables are no longer individually re-listed here). `ledger_entries` was the
first table fixed via option (A) — migrating the database to match the model — rather than (B); see
that log entry for why.
