...
`budget_line_items` — done, verified (check `docs/REMEDIATION_LOG.md` for current deploy status of
the most recent one). 4 of the 66 tables in the original scope remain (see the tier tables above
for the full list; done tables are no longer individually re-listed here). `ledger_entries`,
`budgets`, and
