...
`wht_credit_notes`, `approval_requests`, `approval_decisions`, `budgets` — done, verified (check
`docs/REMEDIATION_LOG.md` for current deploy status of the most recent one). 54 of the 66 tables in
the original scope remain (see the tier tables above for the full list; done tables are no longer
individually re-listed here). `ledger_entries` and `budgets` were fixed via option (A) — migrating
the database to match the model — rather than (B); see those log entries for why.
