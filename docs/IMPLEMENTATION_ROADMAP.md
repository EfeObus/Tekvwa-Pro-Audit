...
- [x] `forensic_audit.py` — 15 + 1 write (`sync_journal_entries_to_ledger`) — 16 total. **Done
      2026-09-26.** All 16 bare `entity_id: uuid.UUID` params (this router's prefix is
      `/{entity_id}/forensic-audit`, mounted under `/api/v1/entities` externally in `main.py`, same
      style as `audit.py`) given `Depends(require_entity_access)`. This router also carries a
      router-level `require_feature([Feature.BENFORDS_LAW])` gate (Intelligence add-on tier), same
      as `budget.py`/`fixed_assets.py`, so verification relied on the AST sweep and a clean import
      check rather than an HTTP smoke test (would 403 on the SKU gate before reaching the access
      check). No pre-existing test file covers this router's endpoints.
- [x] `fx.py` — 10, including the 2 confirmed writes. **Done 2026-09-26.** All 10
      `entity_id: uuid.UUID = Path(...)` params given `Depends(require_entity_access)`, same shape
...
| 18 | P0 | 2 | 2.1 | ✅ **Section 2.1 fully complete** (2026-09-26). All 17 files / 164 endpoints migrated to `require_entity_access`. The single comprehensive parameterized test suite (`tests/test_phase2_comprehensive_entity_isolation.py`, 88 GET endpoints × 2 directions) confirms **100% correctly reject a foreign organization's `entity_id`, zero exceptions** — the actual Finding 18 property, mechanically proven, not just asserted. The "24 confirmed safe" regression bucket the roadmap names (`tests/test_phase2_confirmed_safe_regression.py`) also re-verified clean. Several files got same-root-cause fixes beyond their original per-file tallies, and this pass surfaced 5 severe, unrelated, pre-existing bugs (10+ smaller crash/contract bugs in total) — `consolidation.py` (`group_id`, 17 endpoints via new `require_group_access`), `ml_ai.py` (`detect_anomalies`), `report_template.py` (`clone_template`'s `target_entity_id`), `reports.py` (`subscribe_to_compliance_alerts`), `tax_2026.py` (a `TypeError` crash affecting ~35 endpoints + a router double-prefix), `year_end.py`/`report_export.py` (a shared `except Exception` bug across 17 endpoints), `entities.py` (`restore_entity`'s `ImportError`), `dashboard.py` (a same-file routing collision making `compare_kpis` unreachable — fixed), and 2 more one-line crash fixes (`PAYERecord.tax_amount` → `paye_tax`, hit twice) found via the comprehensive suite's "own entity should work" check. 5 further bugs found by that same check (2 missing `ReportsService` PDF-export methods, 2 router/service parameter contract mismatches in `fixed_assets.py`, 1 nonexistent-enum-value bug in `accounting.py`'s AR aging query) are documented and `xfail`-tracked, not fixed — each needs a real design decision, not a safe rename. A separate, cross-file routing collision in `report_template.py`, a systemic gap in `YearEndClosingService`, and a codebase-wide count of the same `except Exception`-swallowing-`HTTPException` pattern (27 more files, potentially 90+ endpoints) were quantified but deliberately **not** fixed — see each file's roadmap entry and REMEDIATION_LOG.md for full writeups. Documentation step done: `docs/TECHNICAL_ARCHITECTURE.md` §6.2 corrected (was describing a never-implemented RLS policy; now documents `require_entity_access`), `docs/CONTRIBUTING.md` gained a mandatory-pattern section, `docs/DATA_MODEL_ERD.md` cross-references it. Only §2.2 (the `UserEntityAccess` index/unique-constraint migration, blocked on a production data-integrity check this session cannot perform) remains open in Phase 2. |
